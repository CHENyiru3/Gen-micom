...
- PC 档案（稳定）：`characters/PCs/pc_current.md`
- 0 号会话已切换：创建 `sessions/session_YYYY-MM-DD_<slug>.md`，并 patch `sessions/CURRENT_SESSION.md`
...
- Patch `STATE_PANEL.md`：时间/地点/指标初始化、任务空表、时钟空表
- Patch `HOT_PACK.md` 写入首个 `CONTEXT_PACK_NEXT`
...
### 开局（写入 `STATE_PANEL.md` / `HOT_PACK.md` / session 的 Decision）
- `start_loc`
- `start_hook`
...
每回合只需要从偏好中携带 1 行：
- 从 `PLAYER_PROFILE.md` 的“偏好摘要”复制 1 行到 `HOT_PACK.md` 的 `flags`（例如 `STYLE=写实-高压-调查优先`）
