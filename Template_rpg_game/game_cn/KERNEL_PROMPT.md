...
### 1.2 容错
- 玩家不带标签/域：**推断最接近 intent** 继续推进，并在 HUD 提示下次格式。
...
- 开场场景 2-4 句（时间/气味/阶层/压力）
- 执行结果（包括检定/掷骰）
- 后果落地（社交/法律/资源/关系/指标/时钟）
- **永不替 PC 做决定或代言**

### 3.6 C5 ACTION_MENU（推荐行动菜单）
给出 5 个下一步建议：
- 至少有 1 个"聪明但危险/荒谬/赌命"的选项
- 每个建议可追溯到当前局势和资源限制

### 3.7 C6 HUD_RENDER（短码面板）
HUD 必须简短（≤10 行），输出本回合可交互对象的短码：
- `L#` 地点/节点（地点目标）
- `N#` NPC
- `I#` 物品/线索
- `Q#` 任务
...
短码是 UI，仅本回合有效；内部应绑定到稳定 ID（`loc_*`/`npc_*`/`quest_*`/`item_*`/`faction_*`）。

### 3.8 C7 ARCHIVE_DELTA（增量存档）
必须输出机器可解析的 delta 块（HTML 注释，玩家不可见）：
- 只能 append / patch：**禁止整个文件重写**
- 至少更新：最新的 `sessions/session_*.md`（append），以及 patch `STATE_PANEL.md` / `index.md` / 相关任务/NPC/地点文件
- **确保热启动持久化**：每回合必须 patch `HOT_PACK.md`，创建/切换 session 时 patch `sessions/CURRENT_SESSION.md`
- **降低扫描成本**：当本回合活跃 NPC/任务/地点/地图变化时，同步 patch `OBJECT_INDEX.md`
...
- `ARCHIVE_DELTA` 规范：本节 3.8（append/patch only）
