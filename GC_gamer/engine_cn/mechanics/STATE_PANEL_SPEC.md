...
> **用途**：让 `campaigns/<id>/STATE_PANEL.md` 可被增量 patch、可被稳定检索、可长期运行不漂移。  
...
`campaigns/<id>/STATE_PANEL.md` 必须按以下一级标题顺序出现（可为空，但不可缺失）：
...
- **可追溯**：每次重大变更应在最新 `campaigns/<id>/sessions/session_*.md` 的 Decision 中留痕（通过 `ARCHIVE_DELTA` append）。
