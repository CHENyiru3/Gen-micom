...
- 可变状态文件（如 `campaigns/<id>/STATE_PANEL.md`、`campaigns/<id>/HOT_PACK.md`、`campaigns/<id>/PLAYER_PROFILE.md`、`campaigns/<id>/GOVERNANCE_PANEL.md`）只做章节级 patch。
...
- `campaigns/<id>/HOT_PACK.md`：patch 最新 `CONTEXT_PACK_NEXT`
- `campaigns/<id>/STATE_PANEL.md`：patch（只改变化章节）
- `campaigns/<id>/OBJECT_INDEX.md`：patch（只改 active 指针与 1 行摘要）
...
- 表格字段必须保持稳定（见 `engine/mechanics/STATE_PANEL_SPEC.md`、`engine/mechanics/GOVERNANCE_PANEL_SPEC.md`）。
