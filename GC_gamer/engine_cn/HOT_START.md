...
- `campaigns/<id>/STATE_PANEL.md`：**玩家侧常驻面板**（短、可 patch、便于查看）
- `campaigns/<id>/HOT_PACK.md`：**下一回合热启动包**（≤25 行，机器可读；优先读取）
...
1. `campaigns/<id>/HOT_PACK.md`：读取 `<!-- CONTEXT_PACK_NEXT ... -->`（如果存在）
2. `campaigns/<id>/PLAYER_PROFILE.md`：只读“偏好摘要”（≤8 行）
3. `campaigns/<id>/OBJECT_INDEX.md`：只读 active 指针（NPC/Quest/Location/Map 的 1 行摘要）
...
6. `campaigns/<id>/STATE_PANEL.md`：只读时间/指标/任务/NPC/时钟/位置（按需）
...
启动后立刻检查：
- `campaigns/<id>/STATE_PANEL.md` 的指标/位置与最近 Decision 是否一致
...
- `campaigns/<id>/STATE_PANEL.md`：patch（只改变化章节）
- `campaigns/<id>/HOT_PACK.md`：patch（写入最新 `CONTEXT_PACK_NEXT`）
