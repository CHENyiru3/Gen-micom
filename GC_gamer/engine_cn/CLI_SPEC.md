...
> **版本**: v3.0 | **用途**: 玩家输入协议 | **目标**: 减少歧义、提高检索命中率

---

## A) 必须的命令头（行首）

玩家输入必须以命令头开头（行首），否则不推进剧情，仅返回错误提示 + 示例：
...
| 标签 | 别名 | 用途 | 何时用 |
|------|------|------|--------|
| `[ACT]` | `[行动]` | 做事 | 移动/调查/潜行/撬锁/使用物品 |
| `[LOOK]` | `[观察]` | 观察 | 细致检查/搜寻/看图 |
| `[ASK]` | `[对话]` | 说话 | 对话/提问/交涉 |
| `[FIGHT]` | `[战斗]` | 打架 | 进入/继续战斗行动 |
| `[CAST]` | `[施法]` | 施法 | 法术/仪式/超自然能力 |
| `[MANAGE]` | `[管理]` | 后勤 | 物品/休息/钱/统治面板 |
...
触发内心声音时，用 `[ACT]{内心: xxx}`：

| 选项 | 触发条件 |
|------|----------|
| `[ACT]{内心: 学者}` | 知识/推理/观察时 |
| `[ACT]{内心: 创伤}` | 家人/饥荒/普鲁士相关 |
| `[ACT]{内心: 怀疑}` | 信仰/教会/上帝相关 |
| `[ACT]{内心: 疲惫}` | 旅途/饥饿/寒冷时 |
| `[ACT]{内心: 警觉}` | 危险/潜行时 |
| `[ACT]{内心: 情感}` | 心动/亲密/羞耻/嫉妒/思念/失落等情感波动时 |
| `[ACT]{内心: 圣灵}` | 祈祷/忏悔/道德抉择/异象征兆/圣所氛围触发时 |

示例：`[ACT]{内心: 学者}` → DM会插入学者的内心独白
...
### 3) 系统性调查
```
[ACT]{检查地面血迹，推断来了多久} --risk mid --intent "找出最近有人来过的证据"
```
...
### 5) 战斗回合
```
[FIGHT]{睡眠术覆盖门口，然后后撤5米}
```

### 6) 使用物品
```
[ACT]{点燃油灯，照亮通道} --intent "照明"
```
