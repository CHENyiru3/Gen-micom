...
* 或生成“临时实例地图”（/campaigns/<id>/maps/runtime/）供本次会话使用，待后续确认再合并进主图。
...
> 注：本 repo 已创建 `cartridges/<id>/maps/` 目录与 `cartridges/<id>/maps/MAP_INDEX.md`，后续地图更新按本文件第 7 节通过 `ARCHIVE_DELTA` 增量写入。
