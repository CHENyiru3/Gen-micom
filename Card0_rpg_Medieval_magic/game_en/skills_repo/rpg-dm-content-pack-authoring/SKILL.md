...
description: Author and update RPG content packs (lore/NPC/quests/locations/sessions/state) while keeping kernel protocols stable and avoiding plot-driven drift.
---
...
- Put **facts** in content (`lore/`, `characters/`, `locations/`, `quests/`), not in kernel prompts.
...
- **New NPC**: add to `characters/NPCs/npc_roster.md` + create `characters/NPCs/npc_*.md`.
- **New location**: add to `locations/LOCATION_INDEX.md` + create `locations/loc_*.md` (if using per-location files).
- **New quest**: add to `quests/QUEST_LOG.md`; keep status + hooks short.
- **New lore**: add under `lore/CANON/` (static) or `lore/MIST/` (discoverable); update `lore/INDEX.md`.
- **Session logging**: append decisions to latest `sessions/session_*.md` and keep `sessions/SESSION_INDEX.md` updated.
